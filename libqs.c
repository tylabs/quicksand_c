...
//  
//
//  Created by tyler on 2015-07-01. Updated 2018 05 17
//  Copyright 2016 tylabs.com
//
// Dependencies: libzip, yara, zlib, openssl
//
...
void quicksandDropFiles(struct qs_file *qs_file_iterator, struct qs_file **qs_root) {
    ...
    if (qs_file_iterator->data != NULL) {
        ...
        for (i=0; i < conf; i++) {
            ...
            char *nname = malloc(4096);
            snprintf(nname, 4096, "%s%s_%s_%d.qsdump", QUICKSAND_OUT_DIR, (*qs_root)->sha256, qs_file_iterator->sha256, i+1);
...
void quicksandDropObjects(struct qs_file *qs_file_iterator, struct qs_file **qs_root) {
    ...
    if (qs_file_iterator->data != NULL) {
        ...
        if (qs_file_iterator != *qs_root) {
            ...
            char *nname = malloc(4096);
            snprintf(nname, 4096, "%s%s_%s.qsobj", QUICKSAND_OUT_DIR, (*qs_root)->sha256, qs_file_iterator->sha256);
...
unsigned char* quicksand_parse_rtf(const unsigned char *data, unsigned long data_len, unsigned long *new_len)
{
    ...
    for (i = 0; i < data_len; i++) {
        ...
        //binary section
        if (data[i] == '\\' && data[i+1] == 'b' && i+3 < data_len && data[i+2] == 'i' && data[i+3] == 'n' ) {
            //printf("binary tag detected\n");
            int started = 0;
            
            
            
            //advance if spacing
            for (j = i+4; j < data_len; j++){
                if (data[j] == '0') {
                    //do nothing
                } else if ( isgraph(data[j])) {
                    //j--;
                    break;
                }
            }
            
            char *binsize = malloc(32);
            l=0;
            int end = data_len;
            if (data_len >31 + j ) end = 31 + j;
            ...
            binsize[l] = '\0';
            int bin = (int) strtol(binsize, (char **)NULL, 10);
            //printf("need to read %s  %d chars \n", binsize, bin);
            
            
            end = j+1+bin;
            for (j = j+1; j < end; j++){//skip space
                //convert back to hex
                snprintf((char *)(entity+k), 3, "%02x", data[j]);
                //printf("%02x\n", data[j]);
                k+=2;
...
//routine to strip xml content to facilitate dde detection
unsigned char* quicksand_parse_xml(const unsigned char *data, unsigned long data_len, unsigned long *new_len)
{
    unsigned char* entity = malloc(data_len);
    int i, j=0, l=0;
    unsigned long k=0;
    ...
    for (i = 0; i < data_len; i++) {
        ...
        if (data[i] == '<' && data[i+1] != '?') {
                ...
                //todo track string elements and ignore ends within strings
                j = 0;
                for (j = i+1; j < data_len; j++){
                    if (data[j] == '>') {  
...
void quicksand_extract_blocks(const unsigned char *data, unsigned long data_len, struct qs_message *source, struct qs_file **qs_root)
{
    ...
    //printf("looking for hex blocks\n");
    for (i = 0; i < data_len; i++) {
        if (i + 5 < data_len && data[i+1] == '\\' && data[i+2] == '\'') {//nibble drop
            //printf("nibble %d\n", i);
            i+=4;
            ...
            if (j / 2 >= QUICKSAND_MIN_FILE_SIZE) {
                ...
                if (j % 2 == 1) {
                    unsigned char *decoded_rtf2 = malloc(j+1);
                    hex2str(entity+1, j, decoded_rtf2);
                    //printf("odd hex byte sequence auto correction\n");
                    quicksand_do(decoded_rtf2, j/2, quicksand_build_message(str, source->parent, qs_root, QS_FILE_CHILD), qs_root);
...
int quicksand_unzip(const unsigned char *data, unsigned long data_len, struct qs_message *source, struct qs_file **qs_root)
{
    ...
    for (i = 0; i < zip_get_num_entries(za, 0); i++) {
        if (zip_stat_index(za, i, 0, &sb) == 0) {
            ...
            unsigned char * zcontent = calloc(sb.size+1, 1);
            if ((nsize = zip_fread(zf, zcontent, sb.size)) > 0) {
            
               char* str = malloc(261);
               snprintf(str, 261, "%s%s", "zip:", sb.name);
            
               quicksand_do(zcontent, nsize, quicksand_build_message(str, source->parent, qs_root, QS_FILE_CHILD), qs_root);
            }
            //zip_fclose(zf); v011
...
//full version
char quicksandGetStreamEntropy(unsigned char *ch)
{
    int i;
    int zero=0, printable=0, fe=0, spaces=0;
    
    for ( i = 0; i < QUICKSAND_FACTORS; i++ )
    {
        //printf("%d\n",ch[i]);
        if ((int) ch[i] == 0x00)
            zero++;
        else if ((int) ch[i] == 0x20)
            spaces++;
        else if (isprint((int) ch[i]) || (int) ch[i] == 0x0d || (int) ch[i] == 0x0a)
            printable++;
        else if ((int) ch[i] == 255)
            fe++;
    }
    if (zero > QUICKSAND_FACTORS/3) //341+4
        return '0';
    else if (fe > QUICKSAND_FACTORS/3) //341+4
        return 'F';
    else if (spaces > QUICKSAND_FACTORS/3) //341+4
        return 'S';
    else if (printable > QUICKSAND_FACTORS*0.9) //1003+4
        return 'A';
...
//full version
void quicksandIngestKilobyte(struct a_crypto1024* cryptoStat, unsigned char *ch)
{
    int i;
    
    for ( i = 0; i < QUICKSAND_FACTORS; i++ )
    {
        cryptoStat->a[i][(int) ch[i]] += 1;
...
//full version
struct a_crypto1024* quicksandCreateSpectralArray()
{
    struct a_crypto1024* cryptoStat = malloc(sizeof(struct a_crypto1024));
    int i, j;
...
//full version
void quicksandRefactorArray(struct a_crypto1024* cryptoStat)
{
    int i, j, k;
    int **temp;
    
    for ( k = 0;  k < QUICKSAND_SLICES ; k++ )
    {
        //printf("refactoring array to %d\n", QUICKSAND_ARR_FACTORS[k]);
        
        //allocate pointers for integer arrays
        temp = malloc(sizeof(int*)*QUICKSAND_ARR_FACTORS[k]);
        
        //initialization loop
        for ( i = 0; i < QUICKSAND_ARR_FACTORS[k] ; i++ )
        {
            //allocate memory
            temp[i] = malloc(sizeof(int) * 256);
        ...
        for ( i = 0; i < QUICKSAND_FACTORS ; i++ )
        {
            for ( j = 0; j < 256 ; j++ )
            {
                temp[i % QUICKSAND_ARR_FACTORS[k]][j] += cryptoStat->a[i][j];
            }
        }
        
        for ( i = 0; i < QUICKSAND_ARR_FACTORS[k]; i++ )
        {
            cryptoStat->b[k][i] = getMax(temp[i]);
        }
    ...
    for ( k = 1; k < QUICKSAND_SLICES ; k++ )
    {
        
        int check = 0;
        if (QUICKSAND_ARR_FACTORS[k] < 256) check = 2;
        
#ifdef DEBUG
        printf("potential key at %d ", QUICKSAND_ARR_FACTORS[k]);
        for (i = 0; i < QUICKSAND_ARR_FACTORS[k]; i++){
            printf("%02x", cryptoStat->b[k][i]);
        }
        printf("\n");
#endif
        
        cryptoStat->c[k] = KMPSearch(cryptoStat->b[k], QUICKSAND_ARR_FACTORS[k], cryptoStat->b[check], QUICKSAND_ARR_FACTORS[check], locations);
        //&& cryptoStat->c[k]*QUICKSAND_ARR_FACTORS[k] >= QUICKSAND_ARR_FACTORS[check] * 0.9
        if (cryptoStat->c[k] > 1 ) {//issue #32
#ifdef DEBUG
            printf("KMP hits %d for factor %d\n", cryptoStat->c[k], QUICKSAND_ARR_FACTORS[k]);
#endif
            //printf("set\n");
            cryptoStat->d = k;
        } else {
            if (QUICKSAND_ARR_FACTORS[cryptoStat->d] < 1024) //issue #32
                break;
        }
#ifdef DEBUG
        printf("factor %d parent check = %d [seq %d]\n",QUICKSAND_ARR_FACTORS[k], cryptoStat->c[k],check);
#endif
        
        
    }
#ifdef DEBUG
    printf("factor likely is %d bytes\n", QUICKSAND_ARR_FACTORS[cryptoStat->d]);
    for (i = 0; i < QUICKSAND_ARR_FACTORS[cryptoStat->d]; i++){
        printf("%02x", cryptoStat->b[cryptoStat->d][i]);
    }
    printf("\n");
#endif
...
//full release - brute force 01 to 255, rol, not and non zero space
int quicksand_zero_xor(const unsigned char *data, unsigned long data_len, struct qs_message *source) {
    ...
    for (k=1; k <= 255; k++) {
        ...
        for (i = 0; i < data_len-1; i++) {
            if (data[i] != 0x00)
                tryit[i] =  (unsigned char) (data[i] ^ k) ;
            else
                tryit[i] = 0x00;
        }
        ...
        quicksand_yara_mem(tryit, data_len, QUICKSAND_EXE_YARA, "executable",quicksand_build_message("xor",tryfile,(&source->qs_root),QS_FILE_CHILD));
        if (tryfile->count > before) {
            char *keyst = malloc(3);
            snprintf(keyst, 3, "%02x", k);
            ...
            quicksandStore(tryfile, "xorlen", keylen, 0);
            
            quicksandStore(tryfile, "xorkey", keyst, 0);
            quicksandStore(tryfile, "xorzero", keyzero, 0);
            ...
            int rol = 0;
            if (QUICKSAND_ROL == 1) {
                rol = quicksand_rol(tryit, data_len, quicksand_build_message("xor", tryfile, (&source->qs_root), QS_FILE_CHILD));
            }
            ...
            int add = 0;
            if (rol == 0 && QUICKSAND_MATH == 1){
                add = quicksand_addition(tryit, data_len, quicksand_build_message("xor", tryfile, (&source->qs_root), QS_FILE_CHILD));
                ...
                if (add > 0) {
                    ...
                    char *keyzero = "1";
                    quicksandStore(tryfile, "xorzero", keyzero, 0);
                    char *keyadd = malloc(4);
                    snprintf(keyadd, 3, "%d", add);
                    quicksandStore(tryfile, "add", keyadd, 0);
                    return k;
                }
            }
            int not = 0;
            if (add == 0 && QUICKSAND_NOT == 1){
                not = quicksand_not(tryit, data_len, quicksand_build_message("xor", tryfile, (&source->qs_root), QS_FILE_CHILD));
...
int quicksand_xml_try(const unsigned char *data, unsigned long data_len, struct qs_message *source) {
    
    int i, j, k;
    
    unsigned long *new_len = malloc(sizeof(unsigned long));
    unsigned char *tryit = quicksand_parse_xml(data, data_len, new_len);
    ...
    tryfile->count = 0;
    struct qs_message *trymessage = quicksand_build_message("xml", tryfile, (&source->qs_root), QS_FILE_CHILD);
    tryfile->identifier = trymessage->identifier;
        
        
        
    int before = tryfile->count;
    //printf("running yara before %d\n", before);
    quicksand_yara_mem(tryit, *new_len, QUICKSAND_EXPLOITS_YARA, "executable",quicksand_build_message("xml",tryfile,(&source->qs_root),QS_FILE_CHILD));
    if (tryfile->count > before) {
    //printf("got a hit\n");
        char *xml = "1"; 
            
        if (source->qs_root != NULL && quicksandDedup(source->qs_root, quicksandsha256(tryit, *new_len)))
            return 0;
            
            
        insertQSFile(tryfile, tryit, *new_len, source);
        quicksand_hash(tryit, *new_len, trymessage);
            
        quicksandStore(tryfile, "xml", xml, 0);
...
//full version
int quicksand_lookahead_xor(const unsigned char *data, unsigned long data_len, struct qs_message *source) {
    
    int i, j, k;
    
    
    unsigned char *tryit = xorAheadString(data, data_len);
    ...
    tryfile->count = 0;
    struct qs_message *trymessage = quicksand_build_message("xorla", tryfile, (&source->qs_root), QS_FILE_CHILD);
    tryfile->identifier = trymessage->identifier;
    ...
    quicksand_yara_mem(tryit, data_len, QUICKSAND_EXE_YARA, "executable",quicksand_build_message("xorla",tryfile,(&source->qs_root),QS_FILE_CHILD));
    if (tryfile->count > before) {
        char *keyla = "1";
        ...
        quicksandStore(tryfile, "xorla", keyla, 0);
...
//full version
int quicksand_try_xor(const unsigned char *data, unsigned long data_len, struct qs_message *source, struct a_crypto1024* cryptoStat) {
    
    int i, j;
    
    unsigned char *key = malloc(QUICKSAND_ARR_FACTORS[cryptoStat->d] + 1);
    int entropy = 0;
    ...
    {
        //printf("%02x", cryptoStat->b[cryptoStat->d][i]);
        key[i] = (char) cryptoStat->b[cryptoStat->d][i];
        if (cryptoStat->b[cryptoStat->d][i] > 0)
            entropy += 1;
    }
    //printf("\n");
    
    //skip keys of entropy 0 or 1024 where less than 15% entropy
    if (entropy == 0 || (QUICKSAND_ARR_FACTORS[cryptoStat->d] == 1024 && entropy / QUICKSAND_ARR_FACTORS[cryptoStat->d]*100 < 15) ){
        return 0;
    }
    
    
    unsigned char *outstring = malloc(data_len);
    xor_crypt_out(key, QUICKSAND_ARR_FACTORS[cryptoStat->d], data, data_len, outstring);
    ...
    char *keyst = malloc(QUICKSAND_ARR_FACTORS[cryptoStat->d] * 2 + 1);
    for(i = 0,j=0; i < QUICKSAND_ARR_FACTORS[cryptoStat->d]; i++, j+=2)
        snprintf(keyst+j, QUICKSAND_ARR_FACTORS[cryptoStat->d] * 2 + 1 - j, "%02x", key[i]);
    keyst[j] = '\0';
    
    char *keylen = malloc(5);
    snprintf(keylen, 5, "%d", QUICKSAND_ARR_FACTORS[cryptoStat->d]);
    
    int before = tryfile->count;
    
    quicksand_yara_mem(outstring, data_len, QUICKSAND_EXE_YARA, "executable",quicksand_build_message("xor",tryfile,(&source->qs_root),QS_FILE_CHILD));
    if (tryfile->count > before) {
        ...
        int rol = 0;
        if (QUICKSAND_ROL == 1) {
            rol = quicksand_rol(outstring, data_len, quicksand_build_message("xor", tryfile, (&source->qs_root), QS_FILE_CHILD));
        }
        if (rol > 0) {
...
//full version
void quicksand_cryptanalysis(const unsigned char *data, unsigned long data_len, struct qs_message *source)
{
    struct a_crypto1024* cryptoStat;
    int p, a, b, s=0, spacing=0, count;
    char entropy;
    unsigned char k[QUICKSAND_FACTORS+1];
    char *d = malloc(data_len / QUICKSAND_FACTORS + 2);
    
    
    cryptoStat = quicksandCreateSpectralArray();
    
    //read in file
    p = 0;
    a = 0;
    for (b=0; b < data_len / QUICKSAND_FACTORS; b++) {
        
        memcpy(k,data+b*QUICKSAND_FACTORS, QUICKSAND_FACTORS); //no more than len of QUICKSAND_FACTORS
        //printf("check entropy\n");
        k[QUICKSAND_FACTORS] = '\0';
        entropy = quicksandGetStreamEntropy(k);
        
        //printf ("entropy is %d [%d]\n", entropy, b) ;
        //printf("string is %s", k);
        if (entropy == '1') {
            d[s]= '1';
            //printf("check ingest {spacing %d t=%d}\n", spacing, b);
            if (spacing > 5 || a >= QUICKSAND_MEAN_EXEC / QUICKSAND_FACTORS) {
                
                //printf("spacing may indicate a separate area of entropy\n");
                quicksandRefactorArray(cryptoStat);
                //printf("\ncheck ingest {spacing %d t=%d [%d] %s\n", spacing, b, b*QUICKSAND_FACTORS, source->identifier);
                count = quicksand_try_xor(data, data_len, source, cryptoStat);
                
                if (count > 0 ) {
                    //roll back distribution blocking
                    int m, n=0;
                    //printf("rollback\n");
                    for(m = s-1; m-1 >= 0; m--) {
                        if (d[m] == '1' && n == 1)
            ...
            a++;
            quicksandIngestKilobyte(cryptoStat, k);
            spacing = 0;
        } else {
            d[s]= entropy;
            spacing += 1;
    ...
    if (count > 0) {
        ...
        quicksandStore(source->parent, "distribution", d, 0);
        
    } else if (strcmp(source->identifier,"root") == 0) {
        quicksandStore(source->parent, "distribution", d, 0);
    }
...
int quicksand_do(const unsigned char *data, unsigned long data_len,  struct qs_message *source, struct qs_file **qs_root)
{
    ...
    if (data[0] == 'P' && data[1] == 'K') {
        ...
        //full version
            #ifdef REL_FULL
            if (target->parent->count == tbefore) {
                #ifdef DEBUG
                printf("debug:%s: cryptanalysis\n", source->identifier);
                #endif

                quicksand_cryptanalysis(data, data_len, target);
            ...
            #ifdef REL_FULL
            if (QUICKSAND_BRUTE == 1 && target->parent->count == tbefore){
                quicksand_zero_xor(data, data_len, target);
            }
            
            if (QUICKSAND_LOOKAHEAD == 1 && target->parent->count == tbefore){
                quicksand_lookahead_xor(data, data_len, target);
            }
            #endif
        }
        ...
        if (data[0] == '<' && data[1] == '?' && data[2] == 'x' && data[3] == 'm' ){
			...
			quicksand_xml_try(data, data_len, target);
